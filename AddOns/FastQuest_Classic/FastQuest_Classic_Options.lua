-- $Id: FastQuest_Classic_Options.lua 150 2011-12-05 04:15:14Z arithmandar $
--[[
...
--	FQOptionsShowTag:SetChecked(FQD.Tag);
	...
	FQOptionsQuestCompleteSound:SetChecked(FQD.QuestCompleteSound);
	
	FQOptionsQuestFormat:SetValue(FQD.Format);
	FQOptionsSliderButtonPos:SetValue(FQD.ButtonPos);
	
	UIDropDownMenu_Initialize(FQ_SoundDropDownFrame, FastQuest_SoundSelection_Init);
	UIDropDownMenu_SetWidth(FQ_SoundDropDownFrame, 150);
...
--[[
	if(InterfaceOptionsFrame:IsVisible()) then
		...
		InterfaceOptionsFrame_OpenToCategory("FastQuest Classic");
	end
]]
end
...
function FastQuest_Toggle_QuestCompleteSound()
	info = FQ_INFO..FQ_INFO_QUESTCOMPLETESOUND;
	FQD.QuestCompleteSound = FastQuest_ToggleBoolean(FQD.QuestCompleteSound, info)
	FastQuest_Options_Init();
end

function FastQuest_SoundSelection_Init()
	local info;
	local sound_index = 1;

	for i = 1, getn(FQ_Sound_List), 1 do
		info = { };
		info.text = FQ_Sound_List[i];
		info.func = FastQuest_SoundSelection_OnClick;
		UIDropDownMenu_AddButton(info);
		
		if (info.text == FQD.NotificationSoundChoice) then
			sound_index = i;
		end
	end
	UIDropDownMenu_SetSelectedID(FQ_SoundDropDownFrame, sound_index);
end

function FastQuest_SoundSelection_OnClick(self)
	UIDropDownMenu_SetSelectedID(FQ_SoundDropDownFrame, self:GetID());
	FQD.NotificationSoundChoice = FQ_Sound_List[self:GetID()];
end

function FastQuest_SoundSelection_Preview()
	PlaySoundFile("Interface\\AddOns\\FastQuest_Classic\\Sounds\\"..FQD.NotificationSoundChoice..".mp3", "Master");
end
