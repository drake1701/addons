...
function rematch:LoadingDone(unsuccessful)
	...
	if unsuccessful then
		...
		-- set loaded team
		settings.loadedTeamName = loadingTeamName
		settings.loadedNpcID = saved[loadingTeamName][4]
	...
	if rematch:IsVisible() then
		if settings.loadedTeamName and settings.loadedTeamName==rematch.automaticallyShownForTeamName and not settings.AutoShowStay then
			rematch:Hide()
		else
	...
	if not settings.DontWarnMissing and team then
		local missing
		for i=1,3 do
			if type(team[i][1])=="number" and team[i][1]~=0 then
				missing = true
			end
		end
		if missing then
