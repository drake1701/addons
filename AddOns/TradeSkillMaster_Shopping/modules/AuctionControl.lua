...
function private:ShowConfirmationFrame(mode)
	...
	if mode == "Buyout" then
		...
		if mode == "Post" then
			AuctionFrameAuctions.duration = AuctionFrameAuctions.duration or TSM.db.profile.postDuration
...
function private:CreateConfirmationFrame(parent)
	...
	frame.SetPostMode = function(self, isPostMode)
		if isPostMode then
			...
			self.postFrame.durationDropdown:SetValue(TSM.db.profile.postDuration)
			self.postFrame:Show()
		else
			self:SetWidth(300)
			self:SetHeight(150)
	...
	postFrame.durationDropdown = durationDropdown
